...
func runWorkflowMain(
	...
	if runRecord {
		...
		if flags.MeasureTime && fuzzing < 1 {
			...
			if err := command.RunCommand(origStdout, origStderr, command.NoTimeout, paths.Go, execPath); err != nil {
				headerRemoverMain(paths.Prog)
			}
	...
	if runReplay {
		...
		for _, trace := range rewrittenTraces {
			...
			execPath := paths.MakePathLocal(flags.ExecName)
			command.RunCommand(origStdout, origStderr, command.NoTimeout, paths.Go, execPath)
