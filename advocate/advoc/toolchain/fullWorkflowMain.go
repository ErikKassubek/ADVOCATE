...
func runWorkflowMain(
	...
	if runRecord {
		...
		// Add header
		if err := headerInsertMain(paths.Prog, false, "1", flags.TimeoutReplay, false, fuzzing, fuzzingTrace); err != nil {
	...
	if runReplay {
		...
		for _, trace := range rewrittenTraces {
			...
			if err := headerInsertMain(paths.Prog, true, traceNum, flags.TimeoutReplay, false, fuzzing, fuzzingTrace); err != nil {
				return 0, 0, err
			}
