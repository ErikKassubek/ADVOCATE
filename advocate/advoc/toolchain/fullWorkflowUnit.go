...
import (
	...
	"advocate/utils/results/complete"
...
func unitTestRun(pkg, file, testName string, origStdout, origStderr *os.File) error {
	...
	if flags.TimeoutRecording != -1 {
		...
		err = command.RunCommand(origStdout, origStderr, command.NoTimeout, command.NoCom, "go", "test", "-v", "-timeout", timeoutRecString, "-count=1", "-run="+testName, packagePath)
	} else {
		err = command.RunCommand(origStdout, origStderr, command.NoTimeout, command.NoCom, "go", "test", "-v", "-count=1", "-run="+testName, packagePath)
	}
...
func unitTestRecord(pkg, file, testName string,
	...
	command.RunCommand(osOut, osErr, command.NoTimeout, command.NoCom, paths.Go, "version")

	pkgPath := paths.MakePathLocal(pkg)
	err := command.RunCommand(osOut, osErr, command.NoTimeout, command.NoCom, paths.Go, "test", "-gcflags=all=-N -l", "-v", "-count=1", "-run="+testName, pkgPath)
	if err != nil {
		if isFuzzing {
...
func unitTestReplay(dir, pkg, file,
	...
	for i, trace := range rewrittenTraces {
		...
		pkgPath := paths.MakePathLocal(pkg)
		command.RunCommand(osOut, osErr, command.NoTimeout, command.NoCom, paths.Go, "test", "-gcflags=all=-N -l", "-v", "-count=1", "-run="+testName, pkgPath)
