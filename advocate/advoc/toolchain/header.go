...
// Returns:
//   - error
func headerInsertMain(fileName string, replay bool, replayNumber string,
...
// Returns:
//   - error
func headerInsertUnit(fileName, testName string, replay bool, fuzzing int, replayInfo string, record bool) error {
	if _, err := os.Stat(fileName); os.IsNotExist(err) {
		return fmt.Errorf("file %s does not exist", fileName)
	}
...
func HeaderRemoverDummyMain(importLine int) error {
	...
	for scanner.Scan() {
		...
		if numberOfLinesToSkip > 0 {
			...
			numberOfLinesToSkip = 3
			continue
		} else if currentLine == importLine {
			continue
		} else {
			lines = append(lines, line)
		}
