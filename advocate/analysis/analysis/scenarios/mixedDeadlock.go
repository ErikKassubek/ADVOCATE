...
func chanOpCode(e *trace.ElementChannel) trace.OperationType {
	switch e.GetType(true) {
	case trace.ChannelSend:
		return trace.ChannelSend // Channel: Send
	case trace.ChannelRecv:
		return trace.ChannelRecv // Channel: Receive
	case trace.ChannelClose:
		return trace.ChannelClose // Channel: Close
	default:
		return trace.UnknownOperation
	}
}
...
func makeStuckElem(md mixedCandidate) results.TraceElementResult {
	...
	return results.TraceElementResult{
		...
		ObjID:     md.LockID,
		ObjType:   trace.OperationType("DH"),
		File:      fileR,
		Line:      lineR,
		TPre:      tPreR,
	}
...
func makeContextElems(md mixedCandidate) []results.ResultElem {
	...
	var sendLine int
	var sendTpre int
	sendCode := trace.UnknownOperation
	if md.SendEvent != nil {
