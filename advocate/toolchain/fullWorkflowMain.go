...
func runWorkflowMain(
	...
	if runRecord {
		...
		if flags.MeasureTime && fuzzing < 1 {
			...
			fmt.Printf("%s build\n", paths.Go)
			if err := helper.RunCommand(outFile, outFile, false, paths.Go, "build"); err != nil {
			...
			if err := helper.RunCommand(outFile, outFile, false, execPath); err != nil {
				headerRemoverMain(paths.Prog)
			}
		...
		if err := helper.RunCommand(outFile, outFile, false, paths.Go, "build", "-gcflags=all=-N -l"); err != nil {
			log.Error("Error in building program, removing header and stopping workflow")
			headerRemoverMain(paths.Prog)
			return 0, 0, err
		}
	...
	if runReplay {
		...
		for _, trace := range rewrittenTraces {
			...
			execPath := paths.MakePathLocal(flags.ExecName)
			helper.RunCommand(outFile, outFile, false, execPath)
