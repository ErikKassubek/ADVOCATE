...
func rewriteUnbufChanLeakChanChan(trace *analysis.Trace, bug bugs.Bug) error {
	...
	if stuck.Operation() == analysis.RecvOp { // Case 3
		...
		// add replay signals
		trace.AddTraceElementReplayExitCode(max(bug.TraceElement1[0].GetTSort(), bug.TraceElement2[0].GetTSort())+1, utils.ExitCodeLeakUnbuf)
...
func rewriteBufChanLeak(trace *analysis.Trace, bug bugs.Bug) error {
	...
	if possiblePartner.GetTSort() < stuck.GetTSort() {
		trace.AddTraceElementReplayExitCode(stuck.GetTSort()+1, utils.ExitCodeLeakBuf)
	} else {
		trace.AddTraceElementReplayExitCode(possiblePartner.GetTSort()+1, utils.ExitCodeLeakBuf)
	}
...
func rewriteMutexLeak(trace *analysis.Trace, bug bugs.Bug) error {
	...
	trace.AddTraceElementReplayExitCode(lockOp.GetTPre()+1, utils.ExitCodeLeakMutex)
...
func rewriteWaitGroupLeak(trace *analysis.Trace, bug bugs.Bug) error {
	...
	trace.AddTraceElementReplayExitCode(wait.GetTPre()+1, utils.ExitCodeLeakWG)
...
func rewriteCondLeak(trace *analysis.Trace, bug bugs.Bug) error {
	...
	if len(bug.TraceElement2) == 0 {
		trace.AddTraceElementReplayExitCode(wait.GetTPre()+1, utils.ExitCodeLeakCond)
	} else {
		trace.AddTraceElementReplayExitCode(wait.GetTPre()+1, utils.ExitCodeLeakCond)
	}
