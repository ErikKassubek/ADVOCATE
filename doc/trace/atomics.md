...
the [doc](../../goPatch/src/sync/atomic/doc.go) and [asm](../../goPatch/src/sync/atomic/asm.s) files to [oldName]gocctgo. The same is also done in the
[doc_32](../../goPatch/src/sync/atomic/doc_32.go) and [doc_64](../../goPatch/src/sync/atomic/doc_64.go) files. We add two new files [gocct_atomic.go](../../goPatch/src/sync/atomic/gocct_atomic.go) and [gocct_atomic_type.go](../../goPatch/src/sync/atomic/gocct_atomic_type.go). In them, we now implement a function for each of the
atomic functions with the original function name. Those functions
contain the code for the replay and the [recording](../../goPatch/src/runtime/gocct_trace_atomic.go) and then a call
...
while the [gocct_atomic.go](../../goPatch/src/sync/atomic/gocct_atomic.go)
file contains the functions with the original atomic function names as
shown in the example,
in [gocct_atomic_type.go](../../goPatch/src/sync/atomic/gocct_atomic_type.go),
...
is increased by one. While the functions in [gocct_atomic.go](../../goPatch/src/sync/atomic/gocct_atomic.go) are meant to be used directly by the user,
the function in the [gocct_atomic_type.go](../../goPatch/src/sync/atomic/gocct_atomic_type.go) file are used for the implementation on the
