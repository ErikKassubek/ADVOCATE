...
For the send and receive operations two record functions are implemented. The first one ([AdvocateChanSendPre](../../goPatch/src/runtime/gocct_trace_channel.go#L59)) is called before the send or receive is executed. It records [tPre], [id], [opC], [qSize] and [pos].\
The other function ([AdvocateChanPost](../../goPatch/src/runtime/gocct_trace_channel.go#L126)) record [tPost] and [qCount].\
As a close on a channel cannot block, it only needs one recording function. This function ([AdvocateChanClose](../../goPatch/src/runtime/gocct_trace_channel.go#L95)) records all needed values. For [tPre] and [tPost] the same value is set.
