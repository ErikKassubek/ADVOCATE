...
The creation is then recorded in the old routine with [AdvocateSpawnCaller](../../goPatch/src/runtime/gocct_trace_routine.go#L44).

Here we also create the `gocctRoutineInfo` used to store the trace for the
new routine using the [newAdvocateRoutine](../../goPatch/src/runtime/gocct_routine.go#L44) function.
