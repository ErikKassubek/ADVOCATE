...
//
// File: advocate_replay_strict.go
// Brief: Functions for the replay that strictly follows the trace
//
...
// values for possible expected exit codes
const (
	ExitCodeDefault          = 0
	ExitCodePanic            = 3
	ExitCodeTimeout          = 10
...
// Return a list of all replay codes
func GetPosExitCodes() []int {
	return []int{
		ExitCodeDefault,
		ExitCodePanic,
		ExitCodeTimeout,
		...
		ExitCodeCloseNil,
		ExitCodeNegativeWG,
		ExitCodeUnlockBeforeLock,
		ExitCodeCyclic,
	}
}

// possible replay modes
const (
	replayModePartial = "P"
	replayModeStrict  = "S"
	replayModeSimple  = "W"
...
// Return a list of all replay modes
func GetPosReplayMode() []string {
	return []string{
		replayModeStrict, replayModeSimple, replayModePartial,
	}
}
...
//   - timePre: pre time
//   - file: file in which the operation is executed, for replay elem, mode switch code
//   - line: line number of the operation, for relay elem, exit code
...
//   - ControlModeSwitch bool: only used if op == OperationReplayControl, true if
//     elem contains mode switch (stored in file), false if it contains exit code (stored in line)
...
var (
	...
	// the current replay mode ("S": strict, "W": wait based)
	currentReplayMode string

	// allow skip of stuck elements
	allowImprecise bool
)
...
//
// Parameter
//   - allowImprec bool: if true, the the replay will skip trace elements
//     or release longest waiting elements when it senses that it is stuck
func EnableReplayStrict(allowImprec bool) {
	currentReplayMode = replayModeStrict
	allowImprecise = allowImprec
...
func ReleaseWaits() {
	...
	for {
		...
		if replayElem.Op == OperationReplayControl {
			if replayControl(replayElem) {
				println("BREAK2")
				return
			}
			lastKey = "*"
		...
		if key != lastKey || counter%10000 == 0 {
			lastKey = key
			if printDebug {
				println("\n\n===================\nNext: ", key)
...
//   - chan struct{}: ack chan to report back on when finished
...
func WaitForReplayPath(op Operation, file string, line int, waitForResponse bool) (bool, chan ReplayElement, chan struct{}) {
	...
	// if we are in partial replay mode and the element is not in the
	// remaining trace, just play it
	if currentReplayMode == replayModePartial {
		if !isElementInRemainingTrace(key) {
			if printDebug {
				println("NotInRem2: ", key)
			}
			return false, nil, nil
		}
	}
	...
	if key == nextElem.key() && !waitForAck.waitForAck {
		...
		if replayElem.waitAck {
			if printDebug {
				println("SetWaitForAck: ", key, waitForResponse)
			}
			waitForAck = released{
				replayC:    replayElem,
				replayE:    nextElem,
			...
			if printDebug {
				println("foundReplayElem: ", key, waitForResponse)
			}
...
func getNextReplayElement() (int, ReplayElement) {
	...
	rout := uint64(routine)
	elem := replayData[rout][replayIndex[rout]]
...
// Given the key of an operation that wants to execute, check if this
// element is in the remaining trace
func isElementInRemainingTrace(key string) bool {
	for routine, data := range replayData {
		rout := uint64(routine)
		for _, elem := range data[replayIndex[rout]:] {
			if elem.key() == key {
				return true
			}
		}
...
func foundReplayElement(routine int) {
	...
	if printDebug {
		println("Advance: ", routine, replayIndex[uint64(routine)], "/", len(replayData[uint64(routine)]))
	}
}
