...
// GoCRChanPre adds a channel send/receive to the trace.
//
// Parameters:
...
func GoCRChanPre(id uint64, op Operation, qSize uint, isNil bool) int {
...
// GoCRChanClose adds a channel close to the trace
//
// Parameter:
//   - id uint64: id of the channel
...
func GoCRChanClose(id uint64, qSize uint, qCount uint) int {
	if goCRTracingDisabled {
		return -1
	}
...
// GoCRChanPost sets the operation as successfully finished
//
// Parameters:
//   - index: index of the operation in the trace
//   - c: the channel
//   - op: the operation
func GoCRChanPost(index int, c *hchan, op Operation) {
	if goCRTracingDisabled {
		return
	}
	...
	elem := currentGoRoutineInfo().getElement(index).(GoCRTraceChannel)
...
// GoCRChanPostCausedByClose sets the operation as successfully finished
// Args:
//   - index: index of the operation in the trace
func GoCRChanPostCausedByClose(index int) {
	if goCRTracingDisabled {
		return
	}
...
func (elem GoCRTraceChannel) toString() string {
	opStr := ""
	switch elem.op {
...
func (elem GoCRTraceChannel) toStringForSelect() string {
	opStr := ""
	switch elem.op {
	case OperationChannelSend:
		opStr = "S"
...
func (elem GoCRTraceChannel) getOperation() Operation {
	return elem.op
}
