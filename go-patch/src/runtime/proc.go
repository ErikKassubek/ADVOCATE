...
func newproc(fn *funcval) {
	...
	if file == "<autogenerated>" {
		index = 1
		gp.goCRRoutineInfo = newGoCRRoutine(gp, index)
	}
	...
	if wait {
		...
		if gp.goCRRoutineInfo != nil && notActive {
			if s, ok := spawns[gp.goCRRoutineInfo.replayID]; ok && len(s) > 0 {
				index = s[0]
			}
		}
	}
	if gp.goCRRoutineInfo != nil && (wait || notActive) {
		if s, ok := spawns[gp.goCRRoutineInfo.replayID]; ok && len(s) > 0 {
			spawns[gp.goCRRoutineInfo.replayID] = spawns[gp.goCRRoutineInfo.replayID][1:]
		}
	}
	// GOCP-END
	...
	systemstack(func() {
		...
		newg.goCRRoutineInfo = newGoCRRoutine(newg, index)

		if gp != nil && gp.goCRRoutineInfo != nil {
			GoCRSpawnCaller(gp.goCRRoutineInfo, newg.goCRRoutineInfo.id, file, line)
		}
		// GOCP-END
