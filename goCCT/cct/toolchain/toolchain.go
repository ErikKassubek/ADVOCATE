...
// Brief: Functions to add and remove the GoCCT header into file containing
//    unit tests
//
...
func Run(mode, pathToTest string,
	...
	case "main":
		if paths.GoCCT == "" {
			return 0, 0, fmt.Errorf("Path to gocct required for mode main")
		}
		if paths.Prog == "" {
	...
	case "test", "tests":
		if paths.GoCCT == "" {
			return 0, 0, fmt.Errorf("Path to gocct required")
		}
		if paths.Prog == "" {
