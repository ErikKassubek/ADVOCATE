...
var (
	...
	// timeout
	timeout       = newFlagVal("timeout", "180", "", "Set the timeout in seconds for the recording. To disable set to -1")
...
func PrintHelp() {
	fmt.Println("Welcome to GoCCT")
	fmt.Println("")
	fmt.Println("GoCCT is an analysis tool for concurrent Go programs. It tries to detects concurrency bugs and gives diagnostic insight.")
	fmt.Println("")
	printHeader()
}
...
func printHeader() {
	fmt.Println("Usage: ./gocct [mode] [args]")
	fmt.Println("")
	fmt.Println("GoCCT contains four different mode. These are:")
	fmt.Println("\trecord")
	fmt.Println("\treplay")
