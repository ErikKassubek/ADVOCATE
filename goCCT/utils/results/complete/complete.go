...
import (
	...
	"gocct/utils/consts"
	"gocct/utils/log"
	"gocct/utils/results/explanation"
	"gocct/utils/types"
	"os"
	"strings"
)
...
func printNotExecutedToFiles(elements map[string][]int, selects map[string]map[int][]int,
	...
	path = fmt.Sprintf("%s%sGoCCTNotExecuted", path, consts.Sep)
	...
	pathOperations := fmt.Sprintf("%s%sGoCCTNotExecutedOperations.txt", path, consts.Sep)
	pathSelects := fmt.Sprintf("%s%sGoCCTNotExecutedSelectCases.txt", path, consts.Sep)
