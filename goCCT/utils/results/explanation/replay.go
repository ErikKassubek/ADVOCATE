...
func getOutputCodes() map[string]string {
	if _, err := os.Stat(paths.ResultOutput); os.IsNotExist(err) {
		...
		return map[string]string{"GoCCTFailExplanationInfo": res, "GoCCTFailResplaySucInfo": "information not available"}
	}
...
func getReplayInfo(codes map[string]string, index string) (string, string, string, error) {
	if _, ok := codes["GoCCTFailExplanationInfo"]; ok {
		return "", codes["GoCCTFailExplanationInfo"], codes["GoCCTFailResplaySucInfo"], fmt.Errorf("Could not read output file")
	}
