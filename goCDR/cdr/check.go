...
func CheckBin() error {
	...
	if _, err := os.Stat(path); err != nil && os.IsNotExist(err) {
		...
		return fmt.Errorf("Could not find %s. Run make.bash or make.bat in %s before running gocdr.", path, pathMake)
	}
...
// GoCDR is implemented in and for go1.25. It the analyzed program has another
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(timeLine, "go ") {
			...
			if versionSplit[0] != "1" || versionSplit[1] != "25" {
				errString := "GoCDR is implemented for go version 1.25. "
				...
				errString += "The message 'package gocdr is not in std' in the output.log file may indicate this.\t"
				...
				// errString += `'/home/.../go/pkg/mod/golang.org/toolchain@v0.0.1-go1.23.0.linux-amd64/src/gocdr' or 'package gocdr is not in std' in the output files may indicate an incompatible go version.`
