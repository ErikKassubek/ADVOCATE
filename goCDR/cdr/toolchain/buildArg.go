...
//
// File: header.go
// Brief: Functions to add and remove the GoCDR header into/from files containing
//    a main function
//
...
func getBuildArg(fileName string, replay bool, tracePath string,
	...
	if replay { // replay
		if record {
			buildArg += fmt.Sprintf("-gocdrfuzzing -gocdrpath=%s -gocdrtimeout=%d", tracePath, flags.Timeout)
		} else {
			buildArg += fmt.Sprintf("-gocdrreplay -gocdrpath=%s -gocdrtimeout=%d -gocdratomics=%s", tracePath, replayTimeout, atomicReplayStr)
		}
	} else if fuzzing > 0 {
		buildArg += fmt.Sprintf("-gocdrfuzzing -gocdrpath=%s -gocdrtimeout=%d", fuzzingTrace, replayTimeout)
	} else { // recording
		buildArg += fmt.Sprintf("-gocdrtrace  -gocdrtimeout=%d", replayTimeout)
	}
	...
	for scanner.Scan() {
		...
		if strings.Contains(line, "package main") {
			if static {
				...
				lines = append(lines, "import _ \"gocdr\"")
...
func importRemoveMain() error {
	...
	for scanner.Scan() {
		...
		if strings.Contains(line, "import (") {
			...
			inImportBlock = false
			lines = append(lines, line)
		} else if inImportBlock && strings.Contains(line, "\"gocdr\"") {
			continue
		} else if strings.Contains(line, "import _ \"gocdr\"") {
			continue
		} else {
			lines = append(lines, line)
		}
...
func importRemoverUnit(fileName string) error {
	...
	for scanner.Scan() {
		...
		if strings.Contains(line, "import _ \"gocdr\"") {
			continue
		}
