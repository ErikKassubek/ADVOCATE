...
func runWorkflowUnit(runRecord, runAnalysis, runReplay bool,
	...
	if flags.NotExecuted && flags.ExecName != "" {
		err := complete.Check(filepath.Join(paths.Prog, "gocdrResult"), paths.Prog)
		if err != nil {
...
func unitTestFullWorkflow(
	...
	if runAnalysis {
		...
		err = unitTestAnalyzer(pkgPath, "gocdrTrace", fuzzing, file, testName)
		if err != nil {
			return 0, false, err
		}
