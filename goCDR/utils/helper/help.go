...
var (
	...
	// timeout
	timeout       = newFlagVal("timeout", "600", "", "Set the timeout in seconds for the recording. To disable set to -1")
...
func PrintHelp() {
	fmt.Println("Welcome to GoCDR")
	fmt.Println("")
	fmt.Println("GoCDR is an analysis tool for concurrent Go programs. It tries to detects concurrency bugs and gives diagnostic insight.")
	fmt.Println("")
	printHeader()
}
...
func printHeader() {
	fmt.Println("Usage: ./gocdr [mode] [args]")
	fmt.Println("")
	fmt.Println("GoCDR contains four different mode. These are:")
	fmt.Println("\trecord")
	fmt.Println("\treplay")
...
func printHelpRecord() {
	...
	// timeout
	fmt.Println(timeout.toString(false))
