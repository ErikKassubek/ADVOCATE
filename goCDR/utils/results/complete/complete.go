...
import (
	...
	"gocdr/utils/consts"
	"gocdr/utils/log"
	"gocdr/utils/results/explanation"
	"gocdr/utils/types"
	"os"
	"strings"
)
...
func printNotExecutedToFiles(elements map[string][]int, selects map[string]map[int][]int,
	...
	path = fmt.Sprintf("%s%sGoCDRNotExecuted", path, consts.Sep)
	...
	pathOperations := fmt.Sprintf("%s%sGoCDRNotExecutedOperations.txt", path, consts.Sep)
	pathSelects := fmt.Sprintf("%s%sGoCDRNotExecutedSelectCases.txt", path, consts.Sep)
