...
// This is a bootstrap copy of /home/erik/Arbeit/Advocate/goPatch/src/cmd/compile/internal/pkginit/init.go

//line /home/erik/Arbeit/Advocate/goPatch/src/cmd/compile/internal/pkginit/init.go:1
		...
		if fn.Sym().Name == "init" {
			...
			if fn.Sym().Pkg != nil && fn.Sym().Pkg.Path == "main" &&
				...
				if base.Flag.AdvocateTrace {
					call := ir.NewCallExpr(
						base.AutogeneratedPos,
						ir.OCALL,
						typecheck.LookupRuntime("AdvocateInitTracing"),
						nil,
					)

					call.Args.Append(
						typecheck.DefaultLit(
							ir.NewInt(base.AutogeneratedPos, int64(base.Flag.AdvocateTimeout)),
							types.Types[types.TINT],
						),
					)

					fn.Body = append([]ir.Node{call}, fn.Body...)
				} else if base.Flag.AdvocateReplay {
					call := ir.NewCallExpr(
						base.AutogeneratedPos,
						ir.OCALL,
						typecheck.LookupRuntime("AdvocateInitReplay"),
						nil,
					)

					call.Args.Append(
						ir.NewString(base.AutogeneratedPos, base.Flag.AdvocatePath),
					)
					call.Args.Append(
						typecheck.DefaultLit(
							ir.NewInt(base.AutogeneratedPos, int64(base.Flag.AdvocateTimeout)),
					...
					call.Args.Append(
						ir.NewBool(base.AutogeneratedPos, base.Flag.AdvocateAtomics),
					)
					...
					call := ir.NewCallExpr(
						base.AutogeneratedPos,
						ir.OCALL,
						typecheck.LookupRuntime("AdvocateInitFuzzing"),
						nil,
					)
