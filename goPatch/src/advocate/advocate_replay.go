...
//
// Parameter:
//   - tracePath string: The path to the rewritten trace, relative the the dir where ooscResult is placed
...
//   - replayData *runtime.OoscReplayTrace: the elements are added into this replay data
...
func readTraceFile(fileName string, activeTime map[int]struct{}, activeStart int,
	replayData *runtime.OoscReplayTrace, spawns *map[int][]int, selects *map[string][]runtime.ReplayElement) {
	...
	for scanner.Scan() {
		...
		if op != runtime.OperationNone && !runtime.OoscIgnoreReplay(op, file) {
...
func allTraceElemToRuntimeActive(replayTrace *runtime.OoscReplayTrace) (map[string][]int, int) {
