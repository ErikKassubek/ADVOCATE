...
func MakeTask() {
	...
	for _, fn := range typecheck.Target.Inits {
		if fn.Sym().Name == "init" {
			...
			if isGoCDR && fn.Sym().Pkg != nil && fn.Sym().Pkg.Path == "main" &&
				(fn.Sym().Name == "init" || strings.HasPrefix(fn.Sym().Name, "init.")) {
				if base.Flag.GoCDRTrace {
					call := ir.NewCallExpr(
						base.AutogeneratedPos,
						ir.OCALL,
						typecheck.LookupRuntime("GoCDRInitTracing"),
						nil,
					)

					call.Args.Append(
						typecheck.DefaultLit(
							ir.NewInt(base.AutogeneratedPos, int64(base.Flag.GoCDRTimeout)),
					...
					call := ir.NewCallExpr(
						base.AutogeneratedPos,
						ir.OCALL,
						typecheck.LookupRuntime("GoCDRInitReplay"),
						nil,
					)

					call.Args.Append(
						ir.NewString(base.AutogeneratedPos, base.Flag.GoCDRPath),
					)
					call.Args.Append(
						typecheck.DefaultLit(
							ir.NewInt(base.AutogeneratedPos, int64(base.Flag.GoCDRTimeout)),
					...
					call.Args.Append(
						ir.NewBool(base.AutogeneratedPos, base.Flag.GoCDRAtomics),
					)
					call.Args.Append(
					...
					call := ir.NewCallExpr(
						base.AutogeneratedPos,
						ir.OCALL,
						typecheck.LookupRuntime("GoCDRInitFuzzing"),
						nil,
					)
