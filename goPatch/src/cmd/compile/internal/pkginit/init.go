...
func MakeTask() {
	...
	for _, fn := range typecheck.Target.Inits {
		if fn.Sym().Name == "init" {
			...
			if isGoCCT && fn.Sym().Pkg != nil && fn.Sym().Pkg.Path == "main" &&
				(fn.Sym().Name == "init" || strings.HasPrefix(fn.Sym().Name, "init.")) {
				if base.Flag.GoCCTTrace {
					call := ir.NewCallExpr(
						base.AutogeneratedPos,
						ir.OCALL,
						typecheck.LookupRuntime("GoCCTInitTracing"),
						nil,
					)

					call.Args.Append(
						typecheck.DefaultLit(
							ir.NewInt(base.AutogeneratedPos, int64(base.Flag.GoCCTTimeout)),
							types.Types[types.TINT],
						),
					)
					call.Args.Append(
						ir.NewBool(base.AutogeneratedPos, false),
					)
					...
					call := ir.NewCallExpr(
						base.AutogeneratedPos,
						ir.OCALL,
						typecheck.LookupRuntime("GoCCTInitReplay"),
						nil,
					)

					call.Args.Append(
						ir.NewString(base.AutogeneratedPos, base.Flag.GoCCTPath),
					)
					call.Args.Append(
						typecheck.DefaultLit(
							ir.NewInt(base.AutogeneratedPos, int64(base.Flag.GoCCTTimeout)),
					...
					call.Args.Append(
						ir.NewBool(base.AutogeneratedPos, base.Flag.GoCCTAtomics),
					)
					call.Args.Append(
						ir.NewBool(base.AutogeneratedPos, true),
					)
					...
					call := ir.NewCallExpr(
						base.AutogeneratedPos,
						ir.OCALL,
						typecheck.LookupRuntime("GoCCTInitFuzzing"),
						nil,
					)
