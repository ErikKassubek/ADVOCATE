...
//
// Parameter:
//   - tracePath string: The path to the rewritten trace, relative the the dir where gocctResult is placed
...
//
//go:linkname InitReplay runtime.GoCCTInitReplay
...
func readTraceFile(fileName string,
	replayData *runtime.GoCCTReplayTrace, spawns *map[int][]int, selects *map[string][]runtime.ReplayElement) {
	...
	for scanner.Scan() {
		...
		if op != runtime.OperationNone && !runtime.GoCCTIgnoreReplay(op, file) {
...
//
//go:linkname FinishReplay runtime.GoCCTFinishReplay
func FinishReplay() {
	if r := recover(); r != nil {
...
func allTraceElemToRuntimeActive(replayTrace *runtime.GoCCTReplayTrace) (map[string][]int, int) {
