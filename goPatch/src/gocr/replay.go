...
//
// Parameter:
//   - tracePath string: The path to the rewritten trace, relative the the dir where gocpResult is placed
...
//   - replayData *runtime.GoCRReplayTrace: the elements are added into this replay data
...
func readTraceFile(fileName string, activeTime map[int]struct{}, activeStart int,
	replayData *runtime.GoCRReplayTrace, spawns *map[int][]int, selects *map[string][]runtime.ReplayElement) {
	...
	for scanner.Scan() {
		...
		if op != runtime.OperationNone && !runtime.GoCRIgnoreReplay(op, file) {
