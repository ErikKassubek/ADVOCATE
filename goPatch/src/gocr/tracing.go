...
var tracePathRecorded = "goCRTrace"
...
// The trace is written in the format of goCR.
func FinishTracing() {
	if hasFinished {
...
func writeToTraceFile(routine int, tracePath string) {
	...
	goCRChan := make(chan string)
	go func() {
		runtime.TraceToStringByIDChannel(routine, goCRChan)
		close(goCRChan)
	}()

	// receive the trace and write it to the file
	for trace := range goCRChan {
