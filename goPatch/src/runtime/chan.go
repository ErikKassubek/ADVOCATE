...
type hchan struct {
	...
	ooscIgnore bool   // if true, the channel is ignored by tracing and replay
	// OOSC-END
}
...
func chansend(c *hchan, ep unsafe.Pointer, block bool, callerpc uintptr, ignored bool) bool {
	...
	if !ignored && !c.ooscIgnore {
		wait, ch, _, _ := WaitForReplay(OperationChannelSend, CallerSkipChanSendRecv, false)
		if wait {
			replayElem = <-ch
			if replayElem.Blocked {
				lock(&c.lock)
				_ = OoscChanPre(c.id, OperationChannelSend, c.dataqsiz, false)
				unlock(&c.lock)
	...
	// identify the communication partner in the oosc analysis
	...
	// the post information by OoscChanPost, if 'chansend' returns.
	// ooscIndex is used to connect the post event to the correct
	// pre envent in the trace.
	var ooscIndex int
	if !ignored && !c.ooscIgnore {
		ooscIndex = OoscChanPre(c.id, OperationChannelSend, c.dataqsiz, false)
	}
	// OOSC-END
	...
	if c.closed != 0 {
		// OOSC-START
		if !ignored && !c.ooscIgnore {
			OoscChanPostCausedByClose(ooscIndex)
		}
		// OOSC-END
		unlock(&c.lock)
	...
	if sg := c.recvq.dequeue(); sg != nil {
		...
		send(c, sg, ep, func() {
			if !ignored && !c.ooscIgnore {
				OoscChanPost(ooscIndex, c, OperationChannelSend)
			}
			unlock(&c.lock)
		}, 3)
		// OOSC-END
...
func closechan(c *hchan) {
	...
	// OoscChanClose is called when a channel is closed. It creates a close event
	// in the trace.
	if !c.ooscIgnore {
		...
		OoscChanClose(c.id, c.dataqsiz, c.qcount)
	}
	// OOSC-END
...
func chanrecv(c *hchan, ep unsafe.Pointer, block bool, ignored bool) (selected, received bool) {
	...
	if !ignored && !c.ooscIgnore {
		...
		if wait {
			...
			if replayElem.Blocked {
				lock(&c.lock)
				_ = OoscChanPre(c.id, OperationChannelRecv, c.dataqsiz, false)
				unlock(&c.lock)
				StorePark(unsafe.Pointer(c), CallerSkipChanSendRecv, true)
				BlockForever()
			}
	...
	// post information by OoscChanPost, if 'chansend' returns.
	// ooscIndex is used to connect the post event to the correct
	// pre envent in the trace.
	var ooscIndex int
	if !ignored && !c.ooscIgnore {
		ooscIndex = OoscChanPre(c.id, OperationChannelRecv, c.dataqsiz, false)
	}
	// OOSC-END
	...
	if c.closed != 0 {
		...
		if sg := c.sendq.dequeue(); sg != nil {
			...
			recv(c, sg, ep, func() {
				if !ignored && !c.ooscIgnore {
					OoscChanPost(ooscIndex, c, OperationChannelRecv)
				}
				unlock(&c.lock)
			}, 3)
			// OOSC-END
...
func selectnbsend(c *hchan, elem unsafe.Pointer) (selected bool) {
	...
	if c != nil && !c.ooscIgnore {
		lock(&c.lock)
		ooscIndex = OoscSelectPreOneNonDef(c, true)
		unlock(&c.lock)
	}

	fuzzingEnabled, fuzzingIndex := OoscFuzzingGetPreferredCase(2)
	...
	if c != nil && !c.ooscIgnore {
		lock(&c.lock)
		OoscSelectPostOneNonDef(ooscIndex, res, c)
		unlock(&c.lock)
	}
...
func selectnbrecv(elem unsafe.Pointer, c *hchan) (selected, received bool) {
	...
	if c != nil && !c.ooscIgnore {
		...
		if wait {
			...
			if replayElem.Blocked {
				lock(&c.lock)
				_ = OoscSelectPreOneNonDef(c, false)
				unlock(&c.lock)
				StorePark(unsafe.Pointer(c), CallerSkipSelectOneDef, true)
				BlockForever()
			}
	...
	if c != nil && !c.ooscIgnore {
		lock(&c.lock)
		ooscIndex = OoscSelectPreOneNonDef(c, false)
		unlock(&c.lock)
	}
