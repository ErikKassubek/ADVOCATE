...
type GoCRReplayTrace []ReplayElement
...
func GetReplayTrace() (*GoCRReplayTrace, *map[int][]int, *map[string][]ReplayElement) {
	return &replayData, &spawns, &selects
}

// Print the replay trace for one routine.
func (t GoCRReplayTrace) Print() {
	for _, e := range t {
...
// GoCRIgnoreReplay decides if an operation should be ignored for replay.
...
func GoCRIgnoreReplay(operation Operation, file string) bool {
