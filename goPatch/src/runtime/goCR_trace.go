...
func TraceToStringByID(id uint64) (string, bool) {
	...
	if routine, ok := GoCRRoutines[id]; ok {
...
func TraceIsEmptyByRoutine(routine int) bool {
	lock(&GoCRRoutinesLock)
	defer unlock(&GoCRRoutinesLock)
	if routine, ok := GoCRRoutines[uint64(routine)]; ok {
...
func TraceToStringByIDChannel(id int, c chan<- string) {
	...
	if routine, ok := GoCRRoutines[uint64(id)]; ok {
...
func AllTracesToString() string {
	...
	for i := 1; i <= len(GoCRRoutines); i++ {
		res += ""
		routine := GoCRRoutines[uint64(i)]
		if routine == nil {
			panic("Trace is nil")
		}
...
func DeleteTrace() {
	lock(&GoCRRoutinesLock)
	defer unlock(&GoCRRoutinesLock)
	for i := range GoCRRoutines {
		GoCRRoutines[i].Trace = GoCRRoutines[i].Trace[:0]
	}
}
...
func GoCRIgnore(file string) bool {
	return (containsStr(file, "goPatch/src/") || containsStr(file, "go/pkg/mod")) && !containsStr(file, "goPatch/src/time/tick.go")
}
