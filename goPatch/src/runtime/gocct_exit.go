// GOCCT-FILE_START

// Copyright (c) 2024 Erik Kassubek
//
// File: gocct_exit.go
// Brief: Functionality for the exit codes
//
// Author: Erik Kassubek
// Created: 2025-03-18
//
// License: BSD-3-Clause

package runtime

var gocctExitCode = 0
var gocctExitCodePos = ""

// GetExitCode returns the exit code and exit position
//
// Returns:
//   - int: exit code
//   - string: exit position
func GetExitCode() (int, string) {
	return gocctExitCode, gocctExitCodePos
}

// SetExitCodeFromPanicMsg sets the panic info from the panic message
//
// Parameter:
//   - msg any: the panic message
func SetExitCodeFromPanicMsg(msg any) {
	skip := 3

	switch m := msg.(type) {
	case plainError:
		if m.Error() == "send on closed channel" {
			gocctExitCode = ExitCodeSendClose
			skip = 5
		} else if m.Error() == "close of closed channel" {
			gocctExitCode = ExitCodeCloseClose
			skip = 4
		} else if m.Error() == "close of nil channel" {
			gocctExitCode = ExitCodeCloseNil
			skip = 4
		}
	case string:
		if m == "sync: negative WaitGroup counter" {
			gocctExitCode = ExitCodeNegativeWG
			skip = 5
		} else if hasPrefix(m, "test timed out") || hasPrefix(m, "Timeout") {
			gocctExitCode = ExitCodeTimeout
		} else if m == "sync: unlock of unlocked mutex" {
			gocctExitCode = ExitCodeUnlockBeforeLock
			skip = 6
		} else if m == "sync: Unlock of unlocked RWMutex" {
			gocctExitCode = ExitCodeUnlockBeforeLock
			skip = 4
		} else if m == "sync: RUnlock of unlocked RWMutex" {
			gocctExitCode = ExitCodeUnlockBeforeLock
			skip = 5
		} else if m == "Timeout" {
			gocctExitCode = ExitCodeTimeout
			skip = 0
		}
	default:
		var p _panic
		p.arg = msg
		preprintpanics(&p)
		printpanics(&p)
		print("\n")
		printAllGoroutines()
	}

	_, file, line, _ := Caller(skip)
	gocctExitCodePos = file + posSep + intToString(line)
	if printDebug {
		println("AECP: ", gocctExitCodePos, " ", gocctExitCode)
	}

	if gocctExitCode == 0 {
		gocctExitCode = ExitCodePanic
	}
}
