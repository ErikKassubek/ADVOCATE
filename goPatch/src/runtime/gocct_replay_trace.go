...
type GoCCTReplayTrace []ReplayElement
...
func GetReplayTrace() (*GoCCTReplayTrace, *map[int][]int, *map[string][]ReplayElement) {
	return &replayData, &spawns, &selects
}

// Print the replay trace for one routine.
func (t GoCCTReplayTrace) Print() {
	for _, e := range t {
...
// GoCCTIgnoreReplay decides if an operation should be ignored for replay.
