...
var GoCCTTracingDisabled = true

// var gocctTraceWritingDisabled = false
...
func TraceToChanByID(id uint64) chan string {
	...
	if routine, ok := GoCCTRoutines[id]; ok {
...
func TraceIsEmptyByRoutine(routine int) bool {
	...
	if routine, ok := GoCCTRoutines[uint64(routine)]; ok {
...
func DeleteTrace() {
	lock(&GoCCTRoutinesLock)
	defer unlock(&GoCCTRoutinesLock)
	for i := range GoCCTRoutines {
		GoCCTRoutines[i].Trace = GoCCTRoutines[i].Trace[:0]
	}
}
...
func GoCCTIgnore(file string) bool {
...
func RemoveActive(id uint64) {
	...
	delete(GoCCTRoutines, uint64(id))
}
...
func IsActive(id int) (bool, bool) {
	...
	if g, ok := GoCCTRoutines[uint64(id)]; ok {
