...
// GoCCTMutexPre adds a mutex lock to the trace
//
// Parameter:
...
func GoCCTMutexPre(mem unsafe.Pointer, id uint64, op Operation) int {
...
// GoCCTMutexPost adds the end counter to an operation of the trace.
// For try use GoCCTMutexTryPost.
//
// Parameters:
//   - index: index of the operation in the trace
//   - suc: wether the lock was successfull for try, otherwise true
func GoCCTMutexPost(index int, suc bool) {
	if GoCCTTracingDisabled {
		return
	}
	...
	elem := currentGoRoutineInfo().getElement(index).(GoCCTTraceMutex)
...
func (self GoCCTTraceMutex) isRw() bool {
...
func (self GoCCTTraceMutex) toString() string {
...
func (self GoCCTTraceMutex) opRwToString() (string, string) {
	opStr := ""
	rw := "f"
	switch self.op {
...
func (self GoCCTTraceMutex) getOperation() Operation {
	return self.op
}
...
func (self GoCCTTraceMutex) hasCommit() bool {
	return self.tCom != 0
}
...
func (self GoCCTTraceMutex) resource() []GoCCTTraceResource {
