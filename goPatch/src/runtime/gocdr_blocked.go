...
//
// File: gocdr_partial_deadlock.go
...
func (self *GoCDRG) parkForeverReplay() bool {
	return self.rout.gocdrRoutineInfo.parkForeverReplay
}

func (self *GoCDRG) isNil() bool {
	return self.rout.gocdrRoutineInfo == nil
}

func (self *GoCDRG) id() uint64 {
	return self.rout.gocdrRoutineInfo.id
}

func (self *GoCDRG) goId() uint64 {
	return self.rout.goid
}

func (self *GoCDRG) getForkPos() string {
	return self.rout.gocdrRoutineInfo.GetForkPos()
}

func (self *GoCDRG) setOAT(oat []uint64) {
	self.rout.gocdrRoutineInfo.oat = oat
}

func ForEachGoCDRG(fn func(adGp *GoCDRG)) {
	forEachG(func(gp *g) {
		fn(&GoCDRG{gp})
	})
}
...
// isRoutineWaitingOnConcurrency determines if an gocdr routine is blocked on a concurreny operations
// Returns:
//   - bool: true blocked on concurrency op, false otherwise
func (self *GoCDRG) LastTraceElem() traceElem {
	if self.isNil() {
		return nil
	}

	l := len(self.rout.gocdrRoutineInfo.Trace)

	if l == 0 {
		return nil
	}

	return self.rout.gocdrRoutineInfo.Trace[l-1]
}
