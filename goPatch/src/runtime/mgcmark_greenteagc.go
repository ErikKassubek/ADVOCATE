...
// ADVOCATE-START
// ADD id
func tryDeferToSpanScan(p uintptr, gcw *gcWork, id uint64) bool {
	// ADVOCATE-END
	if useCheckmark {
		return false
	}
	...
	// ADVOCATE-START
	obj := base + uintptr(objIndex)*uintptr(gc.SizeClassToSize[q.class.sizeclass()])
...
func scanObject(b uintptr, gcw *gcWork) {
	...
	for {
		...
		if obj != 0 && obj-b >= n {
			...
			// allocation itself.
			// ADVOCATE-START
			if !tryDeferToSpanScan(obj, gcw, 0) {
				// ADVOCATE-END
