...
func goparkWithTimeout(
	...
	gp.gocdrRoutineInfo.wokenNoTimeout = true
}

func goroutineReadyWithTimeout(arg any, _ uintptr, _ int64) {
	gp := arg.(*g)

	if gp.gocdrRoutineInfo.wokenNoTimeout {
		return
	}

	gp.gocdrRoutineInfo.wokenButTimeout = true
...
func newproc(fn *funcval) {
	...
	if file == "<autogenerated>" {
		...
		gp.gocdrRoutineInfo = newGoCDRRoutine(gp, index, file, line)
	}
	...
	} else if notActive || gocdrFuzzingDelayEnabled {
		// get the spawn info from spawns slice if the fork is directly released by partial replay
		if s, ok := spawns[gp.gocdrRoutineInfo.replayID]; ok && len(s) > 0 {
			index, spawns[gp.gocdrRoutineInfo.replayID] = s[0], spawns[gp.gocdrRoutineInfo.replayID][1:]
	...
	systemstack(func() {
		...
		// GOCDR-START
		newg.gocdrRoutineInfo = newGoCDRRoutine(newg, index, file, line)

		if gp != nil && gp.gocdrRoutineInfo != nil {
			GoCDRSpawnCaller(gp.gocdrRoutineInfo, newg.gocdrRoutineInfo.id, file, line)
		}
		// GOCDR-END
