...
func newproc(fn *funcval) {
	...
	if file == "<autogenerated>" {
		index = 1
		gp.gocdrRoutineInfo = newGocdrRoutine(gp, index, "main", 0)
	}
	...
	if wait {
		...
		if gp.gocdrRoutineInfo != nil && notActive {
			if s, ok := spawns[gp.gocdrRoutineInfo.replayID]; ok && len(s) > 0 {
				index = s[0]
			}
		}
	}
	if gp.gocdrRoutineInfo != nil && (wait || notActive) {
		if s, ok := spawns[gp.gocdrRoutineInfo.replayID]; ok && len(s) > 0 {
			spawns[gp.gocdrRoutineInfo.replayID] = spawns[gp.gocdrRoutineInfo.replayID][1:]
		}
	}
	// GOCDR-END
	...
	systemstack(func() {
		...
		// GOCDR-START
		newg.gocdrRoutineInfo = newGocdrRoutine(newg, index, file, int(line))
