...
func goparkWithTimeout(
	...
	gp.gocctRoutineInfo.wokenNoTimeout = true
}

func goroutineReadyWithTimeout(arg any, _ uintptr, _ int64) {
	gp := arg.(*g)

	if gp.gocctRoutineInfo.wokenNoTimeout {
		return
	}

	gp.gocctRoutineInfo.wokenButTimeout = true
...
func newproc(fn *funcval) {
	...
	if file == "<autogenerated>" {
		minReplayRoutine += 1
		index = minReplayRoutine
		gp.gocctRoutineInfo = newGoCCTRoutine(gp, index, file, line)
	}

	// non internal routine
	if wait {
		elem := <-ch
		index = elem.Index
		defer func() { ack <- struct{}{} }()
	} else if notActive || gocctFuzzingDelayEnabled {
		// get the spawn info from spawns slice if the fork is directly released by partial replay
		if s, ok := spawns[gp.gocctRoutineInfo.replayID]; ok && len(s) > 0 {
			index, spawns[gp.gocctRoutineInfo.replayID] = s[0], spawns[gp.gocctRoutineInfo.replayID][1:]
			if index > minReplayRoutine {
				minReplayRoutine = index
			}
		} else {
			minReplayRoutine += 1
			index = minReplayRoutine
	...
	systemstack(func() {
		...
		// GOCCT-START
		newg.gocctRoutineInfo = newGoCCTRoutine(newg, index, file, line)

		if gp != nil && gp.gocctRoutineInfo != nil {
			GoCCTSpawnCaller(gp.gocctRoutineInfo, newg.gocctRoutineInfo.id, file, line)
		}
		// GOCCT-END
