...
func goroutineReadyWithTimeout(arg any, _ uintptr, _ int64) {
	...
	gp.ooscRoutineInfo.wokenButTimeout = true
...
func newproc(fn *funcval) {
	...
	if file == "<autogenerated>" {
		index = 1
		gp.ooscRoutineInfo = newOoscRoutine(gp, index, file, line)
	}
	...
	if wait {
		...
		if gp.ooscRoutineInfo != nil && notActive {
			if s, ok := spawns[gp.ooscRoutineInfo.replayID]; ok && len(s) > 0 {
				index = s[0]
			}
		}
	}
	if gp.ooscRoutineInfo != nil && (wait || notActive) {
		if s, ok := spawns[gp.ooscRoutineInfo.replayID]; ok && len(s) > 0 {
			spawns[gp.ooscRoutineInfo.replayID] = spawns[gp.ooscRoutineInfo.replayID][1:]
		}
	}
	// OOSC-END
	...
	systemstack(func() {
		...
		// OOSC-START
		newg.ooscRoutineInfo = newOoscRoutine(newg, index, file, line)

		if gp != nil && gp.ooscRoutineInfo != nil {
			OoscSpawnCaller(gp.ooscRoutineInfo, newg.ooscRoutineInfo.id, file, line)
		}
		// OOSC-END
