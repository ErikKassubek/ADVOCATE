...
func printNotExecutedToFiles(elements map[string][]int, selects map[string]map[int][]int,
	...
	path = fmt.Sprintf("%s/gocdrNotExecuted", path)
	...
	pathOperations := fmt.Sprintf("%s/gocdrNotExecutedOperations.txt", path)
	pathSelects := fmt.Sprintf("%s/gocdrNotExecutedSelectCases.txt", path)
