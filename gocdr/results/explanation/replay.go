...
func getOutputCodes() map[string]string {
	if _, err := os.Stat(paths.ResultOutput); os.IsNotExist(err) {
		...
		return map[string]string{"gocdrFailExplanationInfo": res, "gocdrFailResplaySucInfo": "information not available"}
	}
...
func getReplayInfo(codes map[string]string, index string) (string, string, string, error) {
	if _, ok := codes["gocdrFailExplanationInfo"]; ok {
		return "", codes["gocdrFailExplanationInfo"], codes["gocdrFailResplaySucInfo"], fmt.Errorf("Could not read output file")
	}
	...
	if exitCodeInt == 0 {
		...
		replaySuc = consts.ConfirmedTheBug
	}
