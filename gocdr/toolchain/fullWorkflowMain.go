...
// Run gocdr on a program with a main function
//
// Parameter:
...
func runWorkflowMain(
	...
	if firstRun && !flags.KeepResultDir {
	...
	if run || (flags.MeasureTime && fuzzing < 1) {
		...
		fmt.Printf("%s build\n", paths.Go)
		if err := helper.RunCommand(outFile, outFile, paths.Go, "build"); err != nil {
		...
		if err := helper.RunCommand(outFile, outFile, execPath); err != nil {
			headerRemoverMain(paths.Prog)
		}
	...
	if runRecord {
		...
		if err := helper.RunCommand(outFile, outFile, paths.Go, "build", "-gcflags=all=-N -l"); err != nil {
			log.Error("Error in building program, removing header and stopping workflow")
			headerRemoverMain(paths.Prog)
			return 0, 0, err
		}
	...
	// Apply analyzer
	if runAnalysis {
		analyzerOutput := filepath.Join(paths.ProgDir, "gocdrTrace")
	...
	if runReplay {
		...
		for _, trace := range rewrittenTraces {
			...
			if err := headerInserterMain(paths.Prog, true, traceNum, flags.TimeoutReplay, false, fuzzing, fuzzingTrace, outFile); err != nil {
				return 0, 0, err
			}
			...
			execPath := paths.MakePathLocal(flags.ExecName)
			helper.RunCommand(outFile, outFile, execPath)
