...
// Brief: Function to run the whole gocdr workflow, including running,
...
// Run gocdr for all given unit tests
//
// Parameter:
//   - pathTogocdr string: pathTogocdr
...
func runWorkflowUnit(dir string, run, runRecord, runAnalysis, runReplay bool,
	...
	if paths.Gocdr == "" {
		return 0, 0, errors.New("Path to gocdr is empty")
	}
	if dir == "" {
	...
	if firstRun && !flags.Continue {
		if !flags.SkipExisting && !flags.KeepResultDir {
			os.RemoveAll("gocdrResult")
		}

		if info, _ := os.Stat("gocdrResult"); info == nil {
			if err := os.MkdirAll("gocdrResult", os.ModePerm); err != nil {
				return 0, 0, fmt.Errorf("Failed to create gocdrResult directory: %v", err)
			}
		}

		// Remove possibly leftover traces from unexpected aborts that could interfere with replay
		// RemoveTraces(dir)
		if !flags.KeepResultDir {
			removeLogs(dir)
		}
	}
	...
	// resultPath := filepath.Join(dir, "gocdrResult")
	...
	for _, file := range testFiles {
		...
		for _, testFunc := range testFunctions {
			...
			directoryName := paths.SetCurrentResult(currentFile, testNumber, fileNameWithoutEnding, testFunc, runReplay)
			...
			nrReplay, anaPassed, err := unitTestFullWorkflow(paths.Gocdr,
...
func getFilesInResult(dir string) (map[string]struct{}, int, error) {
	...
	path := filepath.Join(dir, "gocdrResult")
...
//
// Parameter:
//   - pathTogocdr string: path to gocdr
...
//   - error
func unitTestFullWorkflow(pathTogocdr, dir string,
	...
	// Validate required inputs
	if pathTogocdr == "" {
		return 0, false, errors.New("Path to gocdr is empty")
	}
	if dir == "" {
	...
	if runAnalysis {
		...
		err = unitTestAnalyzer(pkgPath, "gocdrTrace", fuzzing)
...
func unitTestReplay(dir, pkg, file,
	...
	log.Debug("Start replay execution")
	...
	// log.Debugf("Found %d rewritten traces", len(rewrittenTraces))
	...
	for i, trace := range rewrittenTraces {
		...
		log.Infof("Run replay execution %d/%d", i+1, len(rewrittenTraces))
		pkgPath := paths.MakePathLocal(pkg)
		helper.RunCommand(osOut, osErr, paths.Go, "test", "-gcflags=all=-N -l", "-v", "-count=1", "-run="+testName, pkgPath)
		log.Infof("Finished  replay execution %d/%d", i+1, len(rewrittenTraces))
