...
var (
	...
	keepResultDir = newFlagVal("keepResultDir", "false", "Do not replace old result dirs")
...
func PrintHelp() {
	...
	fmt.Println("")
	fmt.Println("GoCDR is a recording and replay tool for concurrent Go programs.")
	fmt.Println("")
	printHeader()
}
...
func printHelpRecord() {
	...
	fmt.Println(noProgress.toString(false))
	fmt.Println(output.toString(false))
	fmt.Println(keepResultDir.toString(false))
