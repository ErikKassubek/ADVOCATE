...
func pathsGocdr() {
	...
	Gocdr = CleanPathHome(filepath.Dir(filepath.Dir(execPath)))
	GoPatch = filepath.Join(Gocdr, "goPatch")
	Go = filepath.Join(Gocdr, "goPatch", "bin", "go")
...
func SetCurrentResult(fileNumber, testNumber int, fileName, testName string, replayMode bool) string {
	dirName := fmt.Sprintf("file(%d)-test(%d)-%s-%s", fileNumber, testNumber, fileName, testName)
	if replayMode && flags.KeepResultDir {
		dirName += "_replay"
	}
